...
.. note::
    ...
    The vdW radii file defines the radius values for each atom by residue and when not defined, it uses a generic value based on the atom type (see `van der Waals file template <../_cfg_files/vdw_file_template.html>`_).
